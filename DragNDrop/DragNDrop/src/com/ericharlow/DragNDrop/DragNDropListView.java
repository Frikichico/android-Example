...
public class DragNDropListView extends ListView {
	...
	public DragNDropListView(Context context, AttributeSet attrs) {
		...
		//[shravan.mahankali] retrieve the draggerId provided by user in the layout file.
		// "draggerId" is mandatory field.
		TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.DragNDropListView);
		draggerId = ta.getResourceId(R.styleable.DragNDropListView_draggerId, -1);
		
		if(draggerId < 0) {
			throw new IllegalStateException("Provide draggerId to use this view.");
		}
	}
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		if (action == MotionEvent.ACTION_DOWN) {
			...
			if (startPos != INVALID_POSITION) {
                ...
                if(x >= grabber.getLeft() && x <= grabber.getRight() && mDragPointY >= grabber.getTop() && mDragPointY <= grabberLocationOnScreen[1])
                	mDragMode = true;
			}
		}
